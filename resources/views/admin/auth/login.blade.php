...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .stat-grid { position: relative; z-index: 1; margin-top: 2.8rem; display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
    .stat-card { background: rgba(255,255,255,0.025); border: 1px solid var(--border); border-radius: 12px; padding: 14px; transition: border-color 0.25s, background 0.25s; cursor: default; }
    .stat-card:hover { border-color: var(--border-hi); background: rgba(255,255,255,0.04); }
    .stat-val { font-family: 'Syne', sans-serif; font-size: 1.45rem; font-weight: 800; letter-spacing: -0.02em; background: linear-gradient(135deg, var(--text), var(--muted2)); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; }
    .stat-lbl { font-size: 0.7rem; color: var(--muted); margin-top: 4px; line-height: 1.4; }

    .recent-logins { position: relative; z-index: 1; margin-top: 1.8rem; }
    .recent-logins h3 { font-size: 0.68rem; font-weight: 600; color: var(--muted); letter-spacing: 0.1em; text-transform: uppercase; margin-bottom: 10px; }
    .la { width: 30px; height: 30px; border-radius: 50%; border: 2px solid var(--panel2); display: inline-flex; align-items: center; justify-content: center; font-size: 11px; font-weight: 700; margin-right: -7px; flex-shrink: 0; vertical-align: middle; }
    ...
    .la-more { background: rgba(255,255,255,0.06); border: 2px solid var(--border); color: var(--muted2); font-size: 9px; margin-left: 14px; margin-right: 0; }
    .recent-text { font-size: 0.76rem; color: var(--muted2); margin-left: 14px; display: inline; vertical-align: middle; }
    .side-footer { position: relative; z-index: 1; font-size: 0.7rem; color: var(--muted); }

    /* Main */
    .main { flex: 1; display: flex; align-items: center; justify-content: center; padding: 5rem 1.4rem 2rem; position: relative; z-index: 1; }

    .form-card { width: 100%; max-width: 408px; animation: riseIn 0.55s cubic-bezier(0.22,1,0.36,1) both; }
    ...
    .fg label a { color: var(--accent); text-decoration: none; font-size: 0.69rem; text-transform: none; letter-spacing: 0; font-weight: 500; }
    ...
    input[type="email"], input[type="password"], input[type="text"] {
      ...
      color: var(--text); font-family: 'Instrument Sans', sans-serif; font-size: 0.875rem;
    ...
    .custom-cb { width: 16px; height: 16px; border: 1.5px solid var(--border-hi); border-radius: 4px; background: var(--inp); cursor: pointer; appearance: none; -webkit-appearance: none; position: relative; transition: all 0.18s; flex-shrink: 0; }
    .custom-cb:checked { background: var(--accent); border-color: var(--accent); }
    .custom-cb:checked::after { content: ''; position: absolute; left: 3px; top: 0px; width: 5px; height: 9px; border: 2px solid #fff; border-top: none; border-left: none; transform: rotate(45deg); }
    .remember-label { font-size: 0.8rem; color: var(--muted2); cursor: pointer; }
    ...
    .error-box .err-icon { font-size: 13px; flex-shrink: 0; margin-top: 1px; }
    ...
    .submit-btn::before { content: ''; position: absolute; inset: 0; background: linear-gradient(135deg, rgba(255,255,255,0.1), transparent); opacity: 0; transition: opacity 0.2s; }
    ...
    .submit-btn:disabled { opacity: 0.42; cursor: not-allowed; transform: none; }
    ...
    .version-tag { display: flex; align-items: center; justify-content: center; gap: 7px; margin-top: 1.8rem; font-size: 0.68rem; color: var(--muted); }
    .version-tag .vdot { width: 3px; height: 3px; border-radius: 50%; background: var(--muted); }

    @media (max-width: 480px) {
      .form-head h1 { font-size: 1.6rem; }
      .main { padding-top: 5.5rem; padding-bottom: 2rem; align-items: flex-start; }
...
<aside class="side">
  ...
  <div class="side-top">
    ...
    <div class="stat-grid">
      <div class="stat-card"><div class="stat-val">2.4k</div><div class="stat-lbl">Active users today</div></div>
      <div class="stat-card"><div class="stat-val">98.9%</div><div class="stat-lbl">Uptime this month</div></div>
      <div class="stat-card"><div class="stat-val">142</div><div class="stat-lbl">Orders pending</div></div>
      <div class="stat-card"><div class="stat-val">$48k</div><div class="stat-lbl">Revenue this week</div></div>
    </div>
    <div class="recent-logins">
      <h3>Recently Active</h3>
      <div>
        <div class="la la-a">S</div><div class="la la-b">J</div>
        <div class="la la-c">M</div><div class="la la-d">R</div>
...
<main class="main">
  <div class="form-card">
    ...
    <form action="{{ route('admin.login.post') }}" method="POST" id="loginForm" novalidate>
      ...
      <div class="fg">
        ...
        <div class="input-wrap">
          ...
          <input type="email" id="email" name="email" value="{{ old('email') }}" placeholder="admin@example.com" autocomplete="email" required/>
        </div>
      </div>
      ...
      <div class="remember-row">
        <div class="remember-left">
          <input type="checkbox" class="custom-cb" id="remember" name="remember" {{ old('remember') ? 'checked' : '' }}/>
          <label for="remember" class="remember-label">@lang('admin.remember_me')</label>
        </div>
      </div>

      <button type="submit" class="submit-btn" id="submitBtn">
        <span>@lang('admin.sign_in')</span>
        <div class="sp"></div>
      </button>
    </form>
    ...
    <div class="form-footer">
      @lang('admin.no_account')
      <a href="{{ route('admin.register') }}"> @lang('admin.sign_up')</a>
    </div>

    <div class="version-tag">
      <span>E-Commerce Pro</span><span class="vdot"></span>
      <span>v4.0.2</span><span class="vdot"></span>
...
(function(){
  ...
  function updateStrength() {
    const score = calcStrength(pwEl.value);
    const el = document.getElementById('pwStrength');
    if (!pwEl.value) { el.classList.remove('show'); return; }
    el.classList.add('show');
    const lvls = [
      { pct:'20%', color:'#f56565', label: 'Weak' },
      { pct:'45%', color:'#f0b429', label: 'Fair' },
      { pct:'70%', color:'#48bb78', label: 'Good' },
      { pct:'100%',color:'#48bb78', label: 'Strong' },
    ];
    const lvl = lvls[Math.max(0, score - 1)] || lvls[0];
  ...
  document.getElementById('loginForm').addEventListener('submit', function(e) {
    ...
    const pwOk    = pwEl.value.length >= 1;
    let err = false;
    if (!emailOk) { emailEl.classList.add('invalid'); err = true; }
    if (!pwOk)    { pwEl.classList.add('invalid');    err = true; }
